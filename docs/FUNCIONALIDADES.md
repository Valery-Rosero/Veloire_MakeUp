...
1. **Subir Excel**: una hoja por categoría (nombre de hoja = nombre de categoría, sin distinguir tildes/mayúsculas), con columnas de inventario (SKU, Marca, Nombre, Descripción, Tono/Referencia, Cantidad, Precio, Costo unitario) más las de comparación de esa categoría. Botón para descargar la plantilla (con dropdowns reales generados por `exceljs`) desde `/admin/productos/importar/plantilla`. Si un valor no está en la lista permitida, la importación se bloquea con el error exacto (hoja, fila, columna, valor, valores permitidos) — parser y validación en `src/lib/product-import/`. Hojas como "Léeme", "Precios" o "Instrucciones" se ignoran automáticamente (lista en `IGNORED_SHEET_NAMES` en `parser.ts`); cualquier otra hoja que no coincida con una categoría existente no se importa en silencio — se avisa con el nombre exacto de la(s) hoja(s), y desde ese mismo aviso se puede **crear la categoría sin salir del asistente** (nombre, prefijo de SKU, zona del rostro) — al crearla, el archivo ya cargado se vuelve a procesar automáticamente, sin pedir resubirlo (`createCategoryQuick` en `src/app/admin/categorias/actions.ts`). Los campos de comparación específicos de esa categoría (más allá de los universales) se agregan después en `comparison-fields.ts`, ya que son fijos en código.
...
3. **Tonos e imágenes**: igual que antes — imagen principal, color/imagen por tono. El color de cada tono también trae la tira de "colores usados antes" para elegir con un clic.
