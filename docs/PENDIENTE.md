...
- **Versatilidad ante categorías nuevas** (2026-09-16, ampliado 2026-09-21): si el Excel trae una hoja con datos que no coincide con ninguna categoría existente, ya no se pierde en silencio — el Step1 del importador avisa con el nombre exacto de la hoja. Hojas utilitarias conocidas (Léeme, Precios, Instrucciones) se ignoran sin avisar. El match de nombre de hoja↔categoría ignora tildes/mayúsculas. Verificado con el Excel real de inventario del negocio (`Inventario_Veloire_categorizado.xlsx`, 11 hojas, 74 productos): detectó correctamente que faltaba la categoría "Pestañinas" (se creó en Supabase con prefijo `PES`, `face_region: ojos`, campos `formato`/`efecto` en `comparison-fields.ts`) y, tras crearla, 0 hojas sin coincidencia.
- **Crear categoría sin salir del importador** (2026-09-21): desde el mismo aviso de "hoja sin categoría" se puede crear la categoría al vuelo (nombre, prefijo de SKU, zona del rostro) — al confirmar, el Excel ya cargado se reprocesa solo, sin pedir resubirlo. Nueva acción `createCategoryQuick` en `src/app/admin/categorias/actions.ts` (variante de `saveCategory` sin redirect, pensada para usarse desde un modal/inline). Solo cubre los campos base de la categoría; si necesita campos de comparación propios (más allá de los universales), esos se agregan aparte en `comparison-fields.ts` — es una decisión de código a propósito, no se editan desde el admin.
- **Colores reutilizables al elegir tono** (2026-09-21): en vez de escribir el hex a mano cada vez, el editor de color de tonos (formulario de producto y paso 3 del importador) muestra una tira de "colores usados antes" — los hex ya usados en otros tonos, ordenados por frecuencia — para elegir con un clic (`RecentColorSwatches`, alimentado por `getRecentShadeColors` en `src/app/admin/productos/actions.ts`). El gris neutro de "sin variación de color" (`#C8C8C8`) se excluye de la lista a propósito, porque es un valor de respaldo del sistema, no una elección real.
- **Rediseño de "Tonos disponibles" en el formulario de producto** (2026-09-21): el panel de agregar/editar tono (nombre, color, foto, stock, activo en un formulario grande que se expandía) se sentía pesado para algo que debería ser rápido. Se reemplazó por una grilla compacta de círculos de color editables in situ (`ShadeSwatch.tsx`) — nombre y stock se escriben directo bajo el círculo, el color y la foto se editan en popovers chiquitos que se abren sobre el mismo círculo (usan `useClickOutside` para cerrarse), y un ícono de ojo activa/desactiva el tono con un clic. Alcance: solo el formulario manual de producto (`ProductForm.tsx`); el paso 3 del importador de Excel (`Step3Shades.tsx`) no se tocó porque ahí no hay flujo de "agregar tono" (vienen fijos del Excel), así que el problema de bulto no aplicaba igual — se puede extender el mismo diseño ahí si se pide.
- **Barra lateral del admin se quedaba corta en páginas largas** (2026-09-21, bug fix): `AdminSidebar.tsx` usaba `min-h-screen`, que solo garantiza un mínimo de 100vh pero no la estira con el contenido — en una página más alta que la pantalla (ej. el formulario de producto), la barra terminaba a los 100vh y el fondo beige se veía debajo de ella en vez de la barra blanca. Cambiado a `h-screen sticky top-0 overflow-y-auto`: ahora la barra siempre ocupa exactamente el alto de la ventana y queda fija mientras se hace scroll del contenido, sin importar qué tan larga sea la página. De paso se corrigió la barra fija de "Guardar borrador / Publicar producto" (`ProductForm.tsx`, `CategoryForm.tsx`), que usaba `lg:left-60` (240px) en vez de `lg:left-64` (256px, el ancho real del sidebar) — ese desfase de 16px hacía que la barra "mordiera" la esquina del sidebar, más visible ahora que el sidebar es sticky.
